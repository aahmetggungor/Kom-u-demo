...
| Upload malware / resource exhaustion | Dedicated voice path enforces streamed 2 MB limit, PCM WAV structure, duration and energy gates, then stores only AES-GCM ciphertext in tenant RLS quarantine; no download or automatic transcription | Format checks are not malware scanning; isolated decoder/scanner and explicit release remain required |

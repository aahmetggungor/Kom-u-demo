...
Migration `0005_encrypted_audio` stores only bounded WAV metadata, an AES-GCM nonce and ciphertext, a keyed replay digest and a managed key identifier. The root key is environment-provided and HKDF separates encryption and digest keys. Runtime access is tenant-RLS constrained to insert/select; there is no public download route. An existing audio row makes downgrade fail rather than silently discarding ciphertext. Key rotation, quarantine scanning and transcription release remain future migrations/workflows.

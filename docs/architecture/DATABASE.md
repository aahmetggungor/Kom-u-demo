...
Migration `0005_encrypted_audio` stores only bounded WAV metadata, an AES-GCM nonce and ciphertext, a keyed replay digest and a managed key identifier. Migration `0006_audio_scan_release` grants the runtime role updates only to state-machine metadata and adds scanner revision/verdict, optimistic version and human decision fields. The root key is environment-provided and HKDF separates encryption and digest keys. RLS remains forced and there is no public download route. A downgrade refuses to discard scanned/decided state. Key rotation, a real isolated scanner and transcription processing remain future workflows.

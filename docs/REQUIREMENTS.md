...
| KOMSU-FUNC-010 | s.2; s.3 | Belediye hastane, toplanma, barınak, kapalı yol katmanları; React/MapLibre | Sürümlü/lisanslı/checksum'lı Polygon/Multi contract ve stale UI; gerçek belediye paketi eksik | PARTIAL |
...
## Güncel uygulama kanıtı — 2026-09-16
...
| PostgreSQL/PostGIS/pgvector | migrations 0001–0009, gerçek DB testleri; sağlıklı Compose API/DB; yerel restore ve ileri migration kanıtı; encrypted audio/transcript ve GIS metadata | Uzak/offsite restore ve HA eksik; yerel bge-m3 etkin, merkezi süreç yönetimi eksik |
| Konum adayları / GIS katmanları | geolocation.py, map_layers.py, map_package.py, tests/test_geolocation_adapters.py, test_map_layers.py, test_map_package.py; web LocationCandidates/Map | Geocoder fixture ile testli; Polygon/Multi ve paket fallback sentetik; gerçek belediye/tile verisi yok |
...
| Kalite / güvenlik | Toplam 104 Python testi (9 gerçek PG); Python lint, pip-audit ve Bandit; web build/11 test; Android build/lint, 13 unit ve 6 emülatör testi; audio, sınıflandırma, duplicate, geocoder, GIS/style ve çeviri güvenlik dilimleri | Uzak CI çalışmadı; gerçek scanner, bağımsız etiket/native review, fiziksel mikrofon/saha sesi, hukuk onayı ve pentest eksik |
...
Yerel bge-m3 kanıtı: `docs/evaluation/bge-provenance.json`, `bge-diagnostic.json`, `duplicate-retrieval-diagnostic.json`; 12 demo raporu vektörlendi. Altı metinlik tanıda farklı bina benzerliği 0.958, 240 mesaj semantic stress testinde aynı şablon farklı olay 1.0 olmuştur. Çok sinyalli 60/60 olay split'i sentetik adres numarası/konum yardımıyla 1.0 verdi ve terfi edilmedi; otomatik birleştirme yapılmaz. Çeviri kanıtı `translation-provenance.json`, `translation-diagnostic.json`, `translation-pipeline-smoke.json`, `translation-review-package/` ve `TRANSLATION.md`; ses kanıtı `whisper-provenance.json`, `whisper-diagnostic.json` ve `TRANSCRIPTION.md` içindedir. Android altı Room enstrümantasyon testi geçmiştir.

...
function CasePanel({detail,cases,token,teams,allowed,copy,locale,onClose,onChange,onError}:{detail:Detail;cases:Case[];token:string;teams:Team[];allowed:boolean;copy:Catalogue;locale:UiLocale;onClose:()=>void;onChange:(id?:string)=>Promise<void>;onError:(e:unknown)=>void}){
  ...
  return <aside className="detail" aria-label={copy.caseDetails}>
    ...
    {detail.reports.map(r=><article className="original" key={r.id}><div className="eyebrow">{copy.originalLabel} · {r.original_language.toUpperCase()} · {r.source}</div><p lang={r.original_language}>{r.original_text}</p>{r.address_raw&&<small>{t('addressDescription',{value:r.address_raw})}</small>}<TranslationBlock report={r} language={language} copy={copy}/><TranscriptBlock key={r.id+':'+(r.transcript?.version||0)} report={r} token={token} allowed={allowed} copy={copy} onChange={onChange} onError={onError}/><small>{t('processing',{status:r.processing_status,version:r.analysis.pipeline_version||copy.pending})}</small></article>)}
...
function TranscriptBlock({report,token,allowed,copy,onChange,onError}:{report:Report;token:string;allowed:boolean;copy:Catalogue;onChange:()=>Promise<void>;onError:(e:unknown)=>void}){
  const transcript=report.transcript,[text,setText]=useState(transcript?.corrected_text||transcript?.original_text||''),[language,setLanguage]=useState(transcript?.corrected_language||transcript?.language||report.original_language),[reason,setReason]=useState(''),[busy,setBusy]=useState(false);
  if(!report.audio)return null;
  async function save(){
    if(!transcript)return;
    setBusy(true);
    try{await request(token,`/reports/${report.id}/audio/transcript/review`,{expected_version:transcript.version,corrected_text:text,language,reason});await onChange();}
    catch(e){onError(e);}finally{setBusy(false);}
  }
  return <section className="transcript"><strong>{copy.audioTranscript}</strong><small>{formatText(copy.audioState,{state:report.audio.state})}</small>
    {!transcript&&<p>{copy.transcriptUnavailable}</p>}
    {transcript&&transcript.state!=='DONE'&&<p>{formatText(copy.transcriptProgress,{state:transcript.state,attempts:transcript.attempts})}{transcript.error_code?` · ${transcript.error_code}`:''}</p>}
    {transcript?.state==='DONE'&&<><div className="transcript-source"><b>{copy.machineTranscript}</b><p lang={transcript.language||'und'}>{transcript.original_text}</p><small>{transcript.model_revision} · {copy.transcriptConfidenceUnavailable}{transcript.warnings.length?` · ${transcript.warnings.join(', ')}`:''}</small></div>{transcript.corrected_text&&<div className="transcript-corrected"><b>{copy.reviewedTranscript}</b><p lang={transcript.corrected_language||'und'}>{transcript.corrected_text}</p><small>{transcript.correction_reason}</small></div>}{allowed&&<div className="transcript-review"><label>{copy.transcriptLanguage}<select value={language} onChange={e=>setLanguage(e.target.value)}><option value="tr">Türkçe</option><option value="el">Ελληνικά</option><option value="en">English</option><option value="und">{copy.unknown}</option></select></label><label>{copy.transcriptCorrection}<textarea value={text} onChange={e=>setText(e.target.value)} maxLength={8000}/></label><label>{copy.transcriptCorrectionReason}<textarea value={reason} onChange={e=>setReason(e.target.value)} maxLength={500}/></label><button disabled={busy||!text.trim()||reason.trim().length<3} onClick={save}>{busy?copy.sending:copy.saveTranscript}</button></div>}</>}
  </section>;
}

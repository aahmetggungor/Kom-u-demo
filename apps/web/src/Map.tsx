...
import {request,type Case,type MapLayer,type MapPackage} from './api';
...
export function CoordinationMap({cases,onSelect,token,copy,locale}:{cases:Case[];onSelect:(id:string)=>void;token:string;copy:Catalogue;locale:UiLocale}) {
  ...
  const [mapPackage,setMapPackage]=useState<MapPackage|null|undefined>(undefined);
  useEffect(()=>{
    ...
    request<MapPackage>(token,'/map/package',undefined,controller.signal).then(setMapPackage).catch(()=>{if(!controller.signal.aborted)setMapPackage(null);});
    return()=>controller.abort();
  },[token,copy.layerFailed]);
  useEffect(()=>{
    if(!element.current||mapPackage===undefined)return;
    setReady(false);let instance:maplibregl.Map;let resize:ResizeObserver|undefined;
    try{
      instance=new maplibregl.Map({container:element.current,center:[27.4,38.0],zoom:6,style:mapPackage?.enabled&&mapPackage.style?mapPackage.style:{version:8,sources:{},layers:[{id:'background',type:'background',paint:{'background-color':'#e5edec'}}]},attributionControl:false});
  ...
  },[copy.mapStartFailed,copy.mapUnavailable,mapPackage]);
  useEffect(()=>{
  ...
  useEffect(()=>{
    ...
    for(const layer of layers){
      ...
      if(!instance.getSource(id)){instance.addSource(id,{type:'geojson',data:layer.geojson});instance.addLayer({id:id+'-fill',type:'fill',source:id,filter:['==',['geometry-type'],'Polygon'],paint:{'fill-color':color,'fill-opacity':0.28,'fill-outline-color':color}},'cases');instance.addLayer({id:id+'-line',type:'line',source:id,filter:['==',['geometry-type'],'LineString'],paint:{'line-color':color,'line-width':5}},'cases');instance.addLayer({id:id+'-point',type:'circle',source:id,filter:['==',['geometry-type'],'Point'],paint:{'circle-color':color,'circle-radius':7,'circle-stroke-color':'#fff','circle-stroke-width':2}},'cases');}
      for(const suffix of ['-fill','-line','-point'])instance.setLayoutProperty(id+suffix,'visibility',enabled.includes(layer.id)?'visible':'none');
    }
  },[layers,enabled,ready]);
  const packageLabel=mapPackage?.enabled?formatText(copy.mapPackageActive,{version:mapPackage.package_version||'',license:mapPackage.license_name||'',freshness:mapPackage.is_stale?copy.dataStale:copy.dataCurrent}):copy.noBaseMap;
  return <div className="map-shell"><div className="map" ref={element} aria-label={copy.mapAria}/><div className="map-note"><strong>{copy.locationWorkspace}</strong><span>{error||(ready?packageLabel:copy.mapEngineLoading)}</span></div><div className="gis-controls" aria-label={copy.institutionLayers}><strong>{copy.institutionLayers}</strong>{layerError?<small role="status">{layerError}</small>:layers.length===0?<small>{copy.noInstitutionData}</small>:layers.map(layer=><label key={layer.id}><span><input type="checkbox" checked={enabled.includes(layer.id)} onChange={e=>setEnabled(current=>e.target.checked?[...current,layer.id]:current.filter(id=>id!==layer.id))}/>{layer.name}{layer.is_stale?` · ${copy.dataStale}`:''}</span><small>{formatText(copy.layerLoaded,{source:`${layer.provenance} · ${layer.dataset_version} · ${layer.license_name}`,time:new Date(layer.source_updated_at).toLocaleString(localeTags[locale])})}</small></label>)}</div><div className="map-legend"><span>{formatText(copy.visiblePoints,{count:visiblePoints})}</span><span>{copy.reportedLocation}</span></div></div>;
}
